...
void Lettore(void *arg)
{
    ...
    while (1)
    {
        ...
        sh->RF->done = true;
        if (sh->RF->fileattuale + 1 <= sh->numerofile)
        {
            sh->RF->fileattuale++;
        }
        else
        ...
        if ((err = pthread_cond_signal(&sh->RF->cond[0])) != 0)
        {
            exit_with_err("pthread_cond_signal", err);
        }
        if ((err = pthread_mutex_unlock(&sh->RF->lock)) != 0)
...
void Filtro(void *arg)
{
    ...
    while (1)
    {
        ...
        while (sh->thread_name != sh->RF->turn)
        {
            if ((err = pthread_cond_wait(&sh->RF->cond[sh->thread_name], &sh->RF->lock)) != 0)
            {
                exit_with_err("pthread_cond_wait", err);
        ...
        if (sh->RF->fileattuale <= sh->numerofile)
        {
            sh->RF->turn = sh->RF->fileattuale;
        }
        else
        {
            break;
...
int main(int argc, char **argv)
{
    ...
    if (argc < 3)
    {
        printf("Usa %s -v -i <word> <nomefile>...\n", argv[0]);
        exit(EXIT_FAILURE);
    }
    ...
    for (int i = 0; i < numerofile + 1; i++)
    {
        if ((err = pthread_cond_destroy(&srf->cond[i])) != 0)
        {
            exit_with_err("pthread_cond_destroy", err);
        }
    }
    if ((err = pthread_mutex_destroy(&srf->lock)) != 0)
    {
        exit_with_err("pthread_mutex_destroy", err);
    }

    if ((err = pthread_cond_destroy(&sfm->cond_filter)) != 0)
    {
        exit_with_err("pthread_cond_destroy", err);
    }

    if ((err = pthread_cond_destroy(&sfm->cond_main)) != 0)
    {
        exit_with_err("pthread_cond_destroy", err);
    }

    if ((err = pthread_mutex_destroy(&sfm->lock2)) != 0)
    {
        exit_with_err("pthread_mutex_destroy", err);
    }
